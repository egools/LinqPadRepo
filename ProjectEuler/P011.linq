...
for(int row = 0; row < nums.Length; row++)
{
	...
	{	
		...
		long horiz = 
			nums[row][col] * 
			(col + 1 < nums[row].Length ? nums[row][col + 1] : 1) * 
			(col + 2 < nums[row].Length ? nums[row][col + 2] : 1) * 
			(col + 3 < nums[row].Length ? nums[row][col + 3] : 1);
			
		long vert = 
			nums[row][col] * 
			(row + 1 < nums.Length ? nums[row + 1][col] : 1) * 
			(row + 2 < nums.Length ? nums[row + 2][col] : 1) * 
			(row + 3 < nums.Length ? nums[row + 3][col] : 1);
			
		long diag1 = 
			nums[row][col] * 
			(col + 1 < nums[row].Length && row + 1 < nums.Length ? nums[row + 1][col + 1] : 1) * 
			(col + 2 < nums[row].Length && row + 2 < nums.Length ? nums[row + 2][col + 2] : 1) * 
			(col + 3 < nums[row].Length && row + 3 < nums.Length ? nums[row + 3][col + 3] : 1);
			
		long diag2 = 
			nums[row][col] * 
			(col - 1 > 0 && row + 1 < nums.Length ? nums[row + 1][col - 1] : 1) * 
			(col - 2 > 0 && row + 2 < nums.Length ? nums[row + 2][col - 2] : 1) * 
			(col - 3 > 0 && row + 3 < nums.Length ? nums[row + 3][col - 3] : 1);
		
		$"({row},{col}) horiz: {horiz}, vert: {vert}, diag1: {diag1}, diag2: {diag2}".Dump();
